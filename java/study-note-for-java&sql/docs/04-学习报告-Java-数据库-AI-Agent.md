...
- **MCP（Model Context Protocol）**：Anthropic 发起的"AI 工具标准接口"，已被广泛采用（本助手 Operit 也支持 MCP）
...
- 📌 学习捷径：你可以在 Operit 中直接观察 Agent 的工作方式（工具调用、记忆、包系统、MCP）
